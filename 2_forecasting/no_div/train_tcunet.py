...
def make_images(true, pred, epoch):
    # T,P - bs, nf, nx, ny, nz
    true = true.permute(0,2,1,5,4,3)
    pred = pred.permute(0,2,1,5,4,3)
...
Par['num_epochs'] = 200 # NO+div converged within 200 epochs (= 15400 iterations of weight updates)
...
for epoch in range(num_epochs):
    ...
    for x_idx, t_idx, y_idx in tqdm(train_loader, desc=f'Epoch {epoch + 1}/{num_epochs}'):
        ...
        optimizer.zero_grad()
        y_pred = model(x, t)
        L_data = criterion(y_pred, y_true.to(device))

        L_mass = torch.mean( torch.square( model.sd_block.compute_divergence(y_pred, h_=grid_size) ) )

        lam = Par['lam']
        loss = L_data + lam*L_mass
        ...
        mass_loss += L_mass.item()

        # Update learning rate
        scheduler.step()

    train_loss /= len(train_loader)
    data_loss /= len(train_loader)
    mass_loss /= len(train_loader)
    ref_mass_loss = torch.mean( torch.square( model.sd_block.compute_divergence(y_true, h_=grid_size) ) ).item()
    ...
    make_images(y_true, y_pred, epoch+1)
    ...
    print(time_stamp + f' - Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.4e}, Train (data) Loss: {data_loss:.4e}, Train (mass) Loss: {mass_loss:.4e}, Ref (mass) Loss: {ref_mass_loss:.4e}, Val Loss: {val_loss:.4e}, best model: {best_model_id}, LR: {scheduler.get_last_lr()[0]:.4e}, epoch time: {elapsed_time:.2f}')
