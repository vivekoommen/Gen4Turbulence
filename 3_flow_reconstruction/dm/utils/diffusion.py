...
class ElucidatedDiffusion(nn.Module):
    ...
    def sample(self, self_cond, batch_size = None, num_sample_steps = None, clamp = True):
        ...
        shape = (batch_size, self.channels, self.image_size_h, self.image_size_w, self.image_size_d)
    ...
    @torch.no_grad()
    def deterministic_sample(self, self_cond, noise, batch_size = None, num_sample_steps = None, clamp = True):
        batch_size = self_cond.shape[0]
        num_sample_steps = default(num_sample_steps, self.num_sample_steps)
        ...
        images = init_sigma * noise #torch.randn(shape, device = self.device)
        ...
        for sigma, sigma_next, gamma in tqdm(sigmas_and_gammas, desc = 'sampling time step'):
            ...
            eps = 0 * self.S_noise * torch.randn(shape, device = self.device) # stochastic sampling
    ...
    def forward(self, images, self_cond=None):
        batch_size, c, h, w, d, device, image_size_h, image_size_w, image_size_d, channels = *images.shape, images.device, self.image_size_h, self.image_size_w, self.image_size_d, self.channels

        assert h == image_size_h and w == image_size_w and d == image_size_d, f'height and width of image must be {image_size_h}, {image_size_w}, {image_size_d}'
